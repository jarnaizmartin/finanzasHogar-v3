...
> Última actualización: 16/07/2026 (sesión 71 — **PRUEBA DEL FOUNDER EN ORDENADOR → 4 bugs + AJUSTES REHECHO + 🔴 EL GATE DE CALIDAD NO EXISTÍA**. Los 3 primeros hallazgos del founder eran **el mismo bug encadenado**: `demoData` sembraba el plan SIN `lastApplied` → el motor de recurrentes se disparaba al entrar en Modo Prueba (**medido: `applied=4 duplicates=1`** — avisaba de duplicado E inyectaba 4 movimientos "🔄" en el ejemplo curado); ese modal (z-100) ennegrecía la app bajo el tour (z-9000) → el spotlight destapaba el backdrop, no el icono. Fix: `lastApplied` al sembrar + nómina al día 1 (demo entra en **+3.200/1.176/+2.024**) + `blockingModalOpen` (el tour espera al modal). **Enlaces legales**: sí funcionaban, se abrían DETRÁS del onboarding (200 vs 9999) → portal + z-10050. **Marca/letra en escritorio** (logo 44→58, microcopy 0.75→0.925rem). **AJUSTES rehecho**: el color que detectó el founder era un BUG (`Field` con `#64748b` hardcodeado = `muted` del tema CLARO; afecta a 11 formularios) + 5 bloques (Tú·Idioma·Dinero·Tus datos·La app) + **nombre editable/borrable**. **2 fugas de español** ("Activar seguridad", "Updated hace 8 min"). 🔴 **`npx tsc --noEmit` NO COMPROBABA NADA** (tsconfig raíz de referencias, `files: []`) → "tsc limpio" era falso desde sesiones atrás; **el CI no corre type-check** y `vite build` no comprueba tipos. Real: **611 errores → 251** con 2 líneas; cazó **2 crashes en producción** (ADMIN `t` fuera de scope · borrado selectivo `downloadBackup` huérfano desde s.1). **11 commits pusheados `925b86c..dff6bdb`. 1150 tests.** 🔴 Falta prueba en iPhone + diagnosticar el CI (402 errores de lint y lo ejecuta))
