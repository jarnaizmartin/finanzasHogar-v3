...
> Última actualización: 05/07/2026 (sesión 64 — icono: réplica lograda de la referencia "casa F·N·T" en `finnort-icon-fnthouse-s64.html`; falta elegir variante)
...
| Naming definitivo | ✅ **DECIDIDO: `FinNorT`** (Bloque 2 comercial CERRADO, s.11). Dominios registrados `.com .eu .app .io` · TMview clases 9+36 limpias. Handles `@finnortapp`: ✅ Instagram + Reddit · 🔄 YouTube/X/TikTok/Facebook · ⏳ LinkedIn. ✅ **App renombrada a `FinNorT`** (nombre VISIBLE en APP_NAME, manifest, title, i18n 6 idiomas, exports, TOTP — `b947cc0`+`1b4379c`; **T final en MAYÚSCULA**, las 3 consonantes F·N·T resaltan). ⚠️ Identificadores internos (`app:` de backups/sync/licencias + `ADMIN_PASSWORD`) siguen `'FinanzasHogar'` a propósito. Pendiente (no bloquea): registro formal de marca 9+36 · 🔴 **wordmark F·N·T en color** (falta pintar las consonantes en teal en el header-logo; hoy es texto plano) · subir iconos a handles. **Icono 🔴 ABIERTO (s.64):** "Pico Norte" (s.62) descartado por genérico; los PNGs viejos SIGUEN en `public/` → reemplazar al cerrar el nuevo. Tras descartar dirección NORTE (s.63) y 3 exploraciones de inicio de s.64, el founder trajo una **referencia de stock** (casa = tejado a dos aguas + **F·N·T bold** dentro) y la **repliqué bien en paleta app**: `finnort-icon-fnthouse-s64.html`. Clave: letras grandes/bold metidas bajo el alero (son los muros). Variantes SIN decidir: sobre claro **V1 navy** (fiel) / V2 tejado teal / V3 N teal · app-icon **T1 teal** / T2 blanco+N teal. Falta: founder **elige V+T** y confirma **casing FinNorT** (T mayúscula; en el descarte `finnort-lockup-s64.html` se coló "t" minúscula, NO vale). Método vigente: símbolo derivado del nombre + anclado en LETRA (registrable). Paleta navy+teal `userSpaceOnUse`. Bloqueo de marca real, no técnico de beta |
...
| Último commit | s.64: réplica del icono "casa F·N·T" (`finnort-icon-fnthouse-s64.html`) + 3 HTML de descarte (`finnort-{wordmark,icon-house,lockup}-s64.html`) + docs. **0 líneas de código de la app**, 1137 tests intactos |
