...
> Última actualización: 04/07/2026 (sesión 63 — icono REABIERTO: "Pico Norte" descartado por genérico; explorada dirección NORTE/monograma N, sin decisión)
...
| Naming definitivo | ✅ **DECIDIDO: `FinNorT`** (Bloque 2 comercial CERRADO, s.11). Dominios registrados `.com .eu .app .io` · TMview clases 9+36 limpias. Handles `@finnortapp`: ✅ Instagram + Reddit · 🔄 YouTube/X/TikTok/Facebook · ⏳ LinkedIn. ✅ **App renombrada a `FinNorT`** (nombre VISIBLE en APP_NAME, manifest, title, i18n 6 idiomas, exports, TOTP — `b947cc0`+`1b4379c`; **T final en MAYÚSCULA**, las 3 consonantes F·N·T resaltan). ⚠️ Identificadores internos (`app:` de backups/sync/licencias + `ADMIN_PASSWORD`) siguen `'FinanzasHogar'` a propósito. Pendiente (no bloquea): registro formal de marca 9+36 · 🔴 **wordmark F·N·T en color** (falta pintar las consonantes en teal en el header-logo; hoy es texto plano) · subir iconos a handles. **Icono 🔴 REABIERTO (s.63):** "Pico Norte" (doble chevron holograma s.62) **descartado por estar muy usado en el mercado** (forma genérica). Los PNGs viejos SIGUEN en `public/` → reemplazar al cerrar el nuevo. Método aprendido: lo literal (pico/casa/flecha) está saturado → símbolo **derivado del nombre + anclado en LETRA** (registrable). s.63 exploró: ❌ "F-tejado" (`finnort-icon-fhouse-s63.html`, descartada, la F se pierde) · 🔶 dirección **NORTE/monograma N** (`finnort-icon-north-s63.html` + afinado `finnort-icon-north-refine-s63.html`, acabados 1 estrella separada / 3 sobria mono). Founder aprobó la dirección A pero **NO decidió acabado** ("no me gusta mucho ninguna"). Paleta navy+teal `userSpaceOnUse` se mantiene. NO bloquea la beta técnicamente pero es bloqueo de marca real |
...
| Último commit | s.63: icono reabierto — 3 HTML de exploración (`finnort-icon-{fhouse,north,north-refine}-s63.html`) + docs. **0 líneas de código de la app**, 1137 tests intactos |
