...
| ✅ Gate de calidad (s.73) | **RESUELTO.** `npm run type-check` es **paso BLOQUEANTE del CI** (`2d82f25`) y `00_FOUNDATION.md` §11 ya describe la realidad. Ruta: **611 → 251 (s.71) → 107 (s.72) → 0 (s.73)**, siempre por causa raíz. ⚠️ El paso de **Lint sigue advisory** (`continue-on-error`, **140** errores heredados) y así está anotado en `ci.yml`: el verde del CI **no dice nada sobre lint** hasta que se bajen a 0. **Baselines: 0 tipos · 140 lint · 1174 tests** |
...
| Último commit | s.73: `b1050a9` — **27 commits pusheados `40314ef..b1050a9`** (tipos 107→0 · CI gate real · prueba que ejecuta la app · lint 404→140 · 6 bugs reales). **Tipos 0 · lint 140 · 1174 tests · CI verde.** 🔴 Siguiente: **lint→0 + quitar `continue-on-error`** (sin atajos) · **pruebas del founder en iPhone (ruta crítica de beta, 7 sesiones esperando)**. |
