...
| Último commit | s.70: `5debb88` — arranque como charla + fixes. Commits pusheados `b153cd5..5debb88`: onboarding conversacional 5 slides `c58eba0` · fix coachmark móvil (scrollIntoView+clamp safe-area) `5ec5ef9` · franja blanca iOS + logo/nombre más grandes `37b9a5e` · docs cierre `aa9d840` · **fix rebote Modo Prueba `5debb88`** (seedDemo escribía `fh_demo_onboarded` al final; si el volcado fallaba a medias en iPhone, recargaba en demo sin onboarded → volvía al idioma; blindado: marca primero + enterDemo no cambia a demo si la siembra no cuajó; NO reproducible en desktop). **1148 tests OK.** 🔴 Siguiente: prueba del founder en ordenador + iPhone (onboarding nuevo, rebote demo, coachmark, franja inferior). |
