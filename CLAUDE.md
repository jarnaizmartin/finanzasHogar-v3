...
> Última actualización: 04/07/2026 (sesión 62 — cierre: icono desplegado + rename a FinNorT + fix ADMIN)
...
| Naming definitivo | ✅ **DECIDIDO: `FinNorT`** (Bloque 2 comercial CERRADO, s.11). Dominios registrados `.com .eu .app .io` · TMview clases 9+36 limpias. Handles `@finnortapp`: ✅ Instagram + Reddit · 🔄 YouTube/X/TikTok/Facebook · ⏳ LinkedIn. ✅ **App renombrada a `FinNorT`** (nombre VISIBLE en APP_NAME, manifest, title, i18n 6 idiomas, exports, TOTP — `b947cc0`+`1b4379c`; **T final en MAYÚSCULA**, las 3 consonantes F·N·T resaltan). ⚠️ Identificadores internos (`app:` de backups/sync/licencias + `ADMIN_PASSWORD`) siguen `'FinanzasHogar'` a propósito. Pendiente (no bloquea): registro formal de marca 9+36 · 🔴 **wordmark F·N·T en color** (falta pintar las consonantes en teal en el header-logo; hoy es texto plano) · subir iconos a handles. **Icono ✅ CERRADO (s.62):** se abandonó la casa-letras F·N·T ("de niños" = concepto literal). Definitivo = **"Pico Norte"** — doble chevron ascendente (norte/crecimiento) con **degradado holograma #6** (`userSpaceOnUse`, brilla en la punta y se apaga hacia los extremos) sobre navy `#12253c→#070e18`, a sangre completa. **PNGs exportados** (rasterizados con gstack): `public/favicon.svg` + `android-chrome-{192,512}.png` (drop-in, mismo nombre) + extras en `project/commercial/assets/export-icons-s62/` (1024 social, 180 apple-touch, 32/16). ⚠️ **NO desplegado**: el móvil solo cambia tras push→redeploy→**reinstalar la PWA**. Fuente viva: `finnort-icon-champions-s62.html`. NO bloquea la beta |
...
| Último commit | `1b4379c fix(brand): FinNorT` (s.62: icono desplegado y verificado en móvil + rename a FinNorT + fix ADMIN; app en 1137 tests) |
